...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_version.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggVersion = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_version.dart',
      );

      expect(
        ggVersion,
        isNotNull,
        reason: 'Item for executing bin/gg_version.dart not found',
      );

      expect(ggVersion['name'], 'gg_version.dart');
      expect(ggVersion['type'], 'dart');
      expect(ggVersion['request'], 'launch');
